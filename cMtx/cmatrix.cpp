...
void CMatrixAttach(RwMatrixTag pOtherMat, bool bLink)
{
...
CMatrixCMatrix(CMatrix const &mat)
{
    m_pRwMatrix = NULL;
...
CMatrixCMatrix(RwMatrixTag pOtherMat, bool bLink)
{
    m_pRwMatrix = NULL;
    Attach(pOtherMat, bLink);
}

void CMatrixConvertToEulerAngles(float &ex, float &ey, float &ez, int precision)
{
...
void CMatrixCopyToRwMatrix(RwMatrixTag m) const
{
...
void CMatrixSetScale(float scale)
{
          [s 0 0]
     m =  [0 s 0]
          [0 0 s]
...
void CMatrixSetScale(float sx, float sy, float sz)
{
          [sx 0 0]
     m =  [0 sy 0]
          [0 0 sz]
...
void CMatrixUpdateMatrix(RwMatrixTag pMat)
{
    m_matrix = pMat;
}
...
void CMatrixUpdateRwMatrix(RwMatrixTag pRwMatrix) const
{
    pRwMatrix = m_matrix;
    RwMatrixUpdate(pRwMatrix);
}

void CMatrixSetRotateZOnly(float fZ)
{
    m_matrix.right.z = 0;
    m_matrix.up.z = 0;
    m_matrix.at.x = 0.0;
...
void CMatrixForceUpVector(CVector vecUp)
{
    m_matrix.right.z = vecUp.y  m_matrix.up.x - vecUp.x  m_matrix.up.y;
}

CMatrix CMatrixoperator+=(CMatrix const &mat)
{
    m_matrix = mat.m_matrix;
    if(m_pRwMatrix)
    {
...
void CMatrixSetTranslateOnly(float x, float y, float z)
{
    m_matrix.pos.x = x;
    m_matrix.pos.y = y;
    m_matrix.pos.z = z;
...
void CMatrixSetTranslate(float x, float y, float z)
{
